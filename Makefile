...
GOVERSION := $(shell cat ./.go-version 2>/dev/null || echo "1.24.7")
GOARCH := $(shell go env GOARCH)
GOOS := $(shell go env GOOS)
DISABLE_CGO := CGO_ENABLED=0
PACKAGE_NAME := proofread-test
MAIN_FILE := ./main.go
CURRENT_MAKEFILE_NAME := Makefile
DOCKER_FILE := Dockerfile

override LDFLAGS = -X $(PACKAGE_NAME)/pkg/util.version=${VERSION} \
  -X $(PACKAGE_NAME)/pkg/util.buildDate=${BUILD_DATE} \
  -X $(PACKAGE_NAME)/pkg/util.gitCommit=${GIT_COMMIT} \
  -X $(PACKAGE_NAME)/pkg/util.gitTreeState=${GIT_TREE_STATE}

ifneq ($(GIT_TAG),)
	override LDFLAGS += -X $(PACKAGE_NAME)/pkg/util.gitTag=${GIT_TAG}
endif
...
build:
	@mkdir -p out
	$(GO) build -o out/$(APP_NAME)-$(GOOS)-$(GOARCH) -ldflags '$(LDFLAGS)' $(MAIN_FILE)
