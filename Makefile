...
override LDFLAGS = -X proofread-exhibition/pkg/util.version=$(VERSION) \
-X proofread-exhibition/pkg/util.buildDate=${BUILD_DATE} \
-X proofread-exhibition/pkg/util.gitCommit=${GIT_COMMIT} \
-X proofread-exhibition/pkg/util.gitTreeState=${GIT_TREE_STATE}

ifneq ($(GIT_TAG),)
override LDFLAGS += -X proofread-exhibition/pkg/util.gitTag=${GIT_TAG}
endif
